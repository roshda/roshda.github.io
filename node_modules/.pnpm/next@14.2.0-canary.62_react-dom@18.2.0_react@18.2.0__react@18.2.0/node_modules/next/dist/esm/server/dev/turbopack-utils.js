...
export function processIssues(currentEntryIssues, key, result, throwIssue = false) {
...
export async function handleRouteType({ dev, page, pathname, route, currentEntryIssues, entrypoints, manifestLoader, readyIds, rewrites, hooks }) {
    switch(route.type){
        case "page":
            {
...
export async function handleEntrypoints({ entrypoints, currentEntrypoints, currentEntryIssues, manifestLoader, nextConfig, rewrites, dev }) {
...
export async function handlePagesErrorRoute({ currentEntryIssues, entrypoints, manifestLoader, rewrites, hooks }) {
    if (entrypoints.global.app) {
        const key = getEntryKey("pages", "server", "_app");
        const writtenEndpoint = await entrypoints.global.app.writeToDisk();
        hooks == null ? void 0 : hooks.handleWrittenEndpoint(key, writtenEndpoint);
