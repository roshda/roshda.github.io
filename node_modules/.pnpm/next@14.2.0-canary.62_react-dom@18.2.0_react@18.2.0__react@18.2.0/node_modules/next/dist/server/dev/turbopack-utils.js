...
function processIssues(currentEntryIssues, key, result, throwIssue = false) {
...
async function handleRouteType({ dev, page, pathname, route, currentEntryIssues, entrypoints, manifestLoader, readyIds, rewrites, hooks }) {
    switch(route.type){
        case "page":
            {
...
async function handleEntrypoints({ entrypoints, currentEntrypoints, currentEntryIssues, manifestLoader, nextConfig, rewrites, dev }) {
...
async function handlePagesErrorRoute({ currentEntryIssues, entrypoints, manifestLoader, rewrites, hooks }) {
    if (entrypoints.global.app) {
        const key = (0, _entrykey.getEntryKey)("pages", "server", "_app");
        const writtenEndpoint = await entrypoints.global.app.writeToDisk();
        hooks == null ? void 0 : hooks.handleWrittenEndpoint(key, writtenEndpoint);
