...
My amazing teammate Ronak found [this unresolved issue in the snarkjs repo](https://github.com/iden3/snarkjs/issues/423) which pointed to a vulnerability in the Solidity Groth16 verifier template:
...
Then, from the provided cverecord verification key, I noticed that the field in this challenge was the scalar field of the elliptic curve BN128. I googled to find the modulus q of this field, as it will be fixed, and found it in [this ethereum package](https://github.com/ethereum/py_ecc/blob/main/py_ecc/bn128/bn128_curve.py), and I realized the key to breaking this is just setting q - 1. 

By the vulnerability, any two numbers that differ by a multiple of q represent the same field element to the zk‑SNARK verifier, so
...
Also Ronak is my goat for finding the GitHub issue. 
...
If we collect enough of these (each with a distinct 64-bit prime aᵢ), then by the Chinese Remainder Theorem we can reconstruct s mod the product of all the aᵢ's. Since the flag is only 66 bytes long (~528 bits), and each aᵢ is a 64-bit prime, we just need a few congruenes to exceed the flag’s bit length and recover it.
