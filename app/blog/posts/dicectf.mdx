...
This is once again an interactive choose-your-own-adventure game. Every time the player reached a fork in the tunnel, they were presented with two messages — one survival message ("you continue walking. turn to page X.") and one death message ("you die of a fever.", etc). Only one was meant to be readable by the player, depending on how they interacted with the OT protocol. The server repeated this process 64 times, and in order to reach the end and receive the flag, the player had to correctly pick the survival path each round.

To accomplish this, the server used a 1-out-of-2 oblivious transfer protocol based on RSA. Each round, the server generated two random values, x0 and x1, and expected the client to send back a value v. It then calculated:
...
and encrypted the messages via:
...
where H is SHAKE-256. Importantly, the RSA modulus n and private exponent d were the same across all rounds.
